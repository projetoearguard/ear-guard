...
const DEFAULT_AUDIOS_CATALOG = [
  { id: 'branco', name: 'Ruído Branco', icon: 'noise',     desc: 'Tom neutro suave',          referenceDb: 30 },
  { id: 'rosa',   name: 'Ruído Rosa',   icon: 'softNoise', desc: 'Espectro natural calmante', referenceDb: 30 },
  { id: 'chuva',  name: 'Chuva Suave',  icon: 'rain',      desc: 'Gotas ritmadas',            referenceDb: 30 },
...
const CALIBRATION_SEQUENCE = DEFAULT_AUDIOS_CATALOG.map((item) => ({
  id: item.id,
  name: item.name,
  db: item.referenceDb,
  icon: item.icon,
  file: `static/audios/${item.id}.mp3`,
}));
...
/* ================================================================
   IndexedDB — Persistência de áudios
   ================================================================ */
let _idb = null;
...
function initDB() {
  return new Promise((resolve, reject) => {
    ...
    req.onerror = () => { console.warn('IndexedDB falhou'); resolve(); };
  });
}
...
async function loadAudiosFromDB() {
  ...
  for (const meta of saved) {
    if (meta.isDefault) {
      ...
      // Se blob não encontrado, descarta
    }
  }
  state.audios = result;
  saveAudioMeta();
...
function _normalizeDefaultAudioMeta(meta) {
  ...
  if (typeof meta.id === 'string' && meta.id.startsWith('def_')) {
    meta.id = meta.id.slice(4);
  }
  if (meta.id === 'floresta') {
    meta.id = 'forest';
  }
...
function saveAudioMeta() {
  const meta = state.audios.map(a => ({
    ...
    storedKey: a.storedKey, volume: a.volume, icon: a.icon,
    fileName: a.fileName,
  }));
...
/* ================================================================
   Configuração / Persistência leve
   ================================================================ */
function loadConfig() {
  try {
    ...
    state.emergencyAudioId = c.emergencyAudioId || null;
...
function saveConfig() {
  localStorage.setItem('earguard_config', JSON.stringify({
    ...
    emergencyAudioId: state.emergencyAudioId,
...
function getProfileDefinition(profileId) {
  ...
  if (builtin) {
    ...
    return {
      ...
      dbOffset: override.dbOffset ?? _clamp(base.dbOffset + builtin.dbOffsetDelta, -30, 30),
    };
  }
  ...
  container.innerHTML = profiles.map(profile => `
    <div class="profile-asset-card ${state.activeProfile === profile.id ? 'active' : ''}">
      <div class="profile-asset-info">
        ...
        <div class="profile-asset-meta">${profile.sensitivity} · ${profile.cooldown}s · ${profile.debounce}ms · ${profile.noiseNotifyThreshold} dB</div>
      </div>
...
function saveProfileEditor() {
  ...
  const sensitivity = _getVal('profile-sensitivity-input');
  ...
  const profile = {
    ...
    name,
    sensitivity,
    cooldown: _clamp(cooldown, 0, 60),
    debounce: _clamp(debounce, 100, 5000),
    noiseNotifyThreshold: _clamp(threshold, 0, 150),
    dbOffset: _clamp(offset, -30, 30),
    builtIn: false,
  };
  if (BUILTIN_PROFILES[profile.id]) {
    state.profileOverrides[profile.id] = profile;
...
function loadHistoryFromStorage() {
  ...
  state.dbStat = saved.stats || { max: null, avg: null, triggerCount: 0 };
}
...
function showScreen(name) {
  ...
  if (name === 'configurations') renderProfilesManager();
  if (name === 'calibration') renderCalibrationStep();
  if (name === 'configurations') renderNotificationPreviewOptions();
}
...
function _checkTriggers(db) {
  ...
  // Nenhum gatilho — se havia um ativo, parar
  if (_currentTriggerAudioId && state.isPlaying) {
    _currentTriggerAudioId = null;
    // Não para automaticamente, deixa o usuário controlar
  }
}
...
function _activateTrigger(trigger, db) {
  ...
  const vol = (trigger.volume || 80) / 100;
  ...
  if (state.notifyOnTrigger) {
    sendNotification(`Gatilho ativado: ${trigger.name}`);
  }
...
function playAudioById(audioId, volume) {
  ...
  if (!audio || !audio.src) { showToast('Áudio não disponível', 'warn'); return; }
  ...
  _audioEl.loop = true;
  _audioEl.volume = volume !== undefined ? volume : (audio.volume || 80) / 100;

  // Fade in
  _audioEl.volume = 0;
  const targetVol = volume !== undefined ? volume : (audio.volume || 80) / 100;
  _audioEl.play().then(() => {
...
function _fadeVolume(el, from, to, duration) {
  ...
  const timer = setInterval(() => {
    current += delta;
    count++;
    if (el) el.volume = Math.max(0, Math.min(1, current));
...
function stopAudioManual() {
  ...
  _currentTriggerAudioId = null;
  setTimeout(() => { updateNowPlayingUI(); renderAudioList(); }, 700);
}
...
function updateNowPlayingUI() {
  ...
  if (thumb && a && a.icon) {
    thumb.innerHTML = audioIconSVG(a.icon);
  } else if (thumb) {
    thumb.innerHTML = `<svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M9 18V6l12-3v12M9 18c0 1.1-.9 2-2 2s-2-.9-2-2 .9-2 2-2 2 .9 2 2zm12-3c0 1.1-.9 2-2 2s-2-.9-2-2 .9-2 2-2 2 .9 2 2z" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
  }
}
...
function triggerEmergency() {
  if (!state.emergencyAudioId) {
    showToast('Defina um som de emergência nas configurações', 'warn');
    return;
  }
  const audio = state.audios.find(a => a.id === state.emergencyAudioId);
  if (!audio) { showToast('Som de emergência não encontrado', 'error'); return; }

  _stopAudio(false);
  playAudioById(audio.id, 0.9);
  addHistoryEvent('warn', 'Modo emergência ativado', null);
  showToast('Emergência ativada — ' + audio.name, 'warn');
}

function openSelectEmergencyModal() {
  const container = document.getElementById('emergency-audio-options');
  container.innerHTML = '';
  if (state.audios.length === 0) {
    container.innerHTML = '<p style="color:var(--text3);font-size:14px;text-align:center;padding:16px">Nenhum áudio na biblioteca</p>';
  } else {
    state.audios.forEach(a => {
      const isSelected = a.id === state.emergencyAudioId;
      const div = document.createElement('div');
      div.className = 'emergency-option';
      div.innerHTML = `
        ${audioIconSVG(a.icon || 'tone')}
        <span class="emergency-option-name">${_esc(a.name)}</span>
        ${isSelected ? '<svg class="emergency-option-check" width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>' : ''}
      `;
      div.onclick = () => {
        state.emergencyAudioId = a.id;
        saveConfig();
        updateEmergencyAudioName();
        closeModal('modal-select-emergency');
        showToast('Som de emergência definido', 'success');
      };
      container.appendChild(div);
    });
  }
  openModal('modal-select-emergency');
}

function updateEmergencyAudioName() {
  const a = state.emergencyAudioId ? state.audios.find(x => x.id === state.emergencyAudioId) : null;
  _setText('emergency-audio-name', a ? a.name : 'Nenhum selecionado');
}
...
function setSensitivity(val) {
  ...
  if (_analyser) {
    _analyser.smoothingTimeConstant = SENSITIVITY_CONFIG[val].smoothing;
  }
  document.querySelectorAll('.sens-btn').forEach(b => {
...
function applyProfile(key) {
  ...
  if (_analyser) {
    _analyser.smoothingTimeConstant = SENSITIVITY_CONFIG[p.sensitivity].smoothing;
  }
  document.querySelectorAll('.sens-btn').forEach(b => {
...
function saveMonitorConfig() {
  ...
  if (isNaN(cooldown) || isNaN(debounce) || isNaN(offset)) {
    showToast('Valores inválidos', 'error'); return;
  }
...
function saveNotifyConfig() {
  ...
  if (isNaN(threshold)) { showToast('Valor inválido', 'error'); return; }
  state.noiseNotifyThreshold = Math.max(0, Math.min(150, threshold));
...
function saveCalibration() {
  ...
  if (isNaN(offset)) { showToast('Valor inválido', 'error'); return; }
  state.dbOffset = Math.max(-30, Math.min(30, offset));
  _setVal('offset-input', state.dbOffset);
  saveConfig();
...
function finishCalibrationFlow() {
  ...
  renderProfilesManager();
  renderCalibrationStep();
  showToast('Ajuste aplicado ao app inteiro', 'success');
}
...
function renderAudioList() {
  ...
  container.innerHTML = state.audios.map(a => {
    const isPlaying = state.currentAudioId === a.id && state.isPlaying;
    return `
...
function openEditVolumeModal() {
  ...
  if (!selectedAudioId) return;
  const a = state.audios.find(x => x.id === selectedAudioId);
  const vol = a ? (a.volume || 80) : 80;
  const slider = document.getElementById('edit-vol-slider');
  if (slider) { slider.value = vol; _setText('edit-vol-val', vol + '%'); }
...
async function removeAudio() {
  ...
  if (state.emergencyAudioId === selectedAudioId) {
    state.emergencyAudioId = null;
    saveConfig();
    updateEmergencyAudioName();
  }
...
async function addAudio() {
  ...
  state.audios.push({
    id, name, isDefault: false, src, storedKey,
    volume: vol, icon: 'tone', fileName: file.name,
  });
...
function addDefaultAudio(defId) {
  ...
  if (state.audios.some(a => a.id === defId)) {
    showToast('Já adicionado', 'warn'); return;
  }

  const src = `static/audios/${(def.file || defId)}.mp3`;

  state.audios.push({
    id: def.id, name: def.name, isDefault: true,
    src, storedKey: null, volume: 70, icon: def.icon, fileName: null,
  });
...
function saveTrigger() {
  ...
  if (!name) { showToast('Digite um nome', 'error'); return; }
  if (isNaN(dbMin) || isNaN(dbMax)) { showToast('Informe os valores de dB', 'error'); return; }
  if (dbMin >= dbMax) { showToast('dB mínimo deve ser menor que o máximo', 'error'); return; }
  if (!audioId) { showToast('Selecione um áudio', 'error'); return; }

  if (editingTriggerId) {
    const t = state.triggers.find(x => x.id === editingTriggerId);
    if (t) { t.name = name; t.dbMin = dbMin; t.dbMax = dbMax; t.audioId = audioId; t.volume = volume; t.enabled = enabled; }
  } else {
    state.triggers.push({
      id: 'trig_' + Date.now(),
      name, dbMin, dbMax, audioId, volume, enabled,
    });
  }
